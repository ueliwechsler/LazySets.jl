...
Let ``B`` be a ball in the infinity norm with center ``c`` and radius ``r``.
...
```math
    ρ(d, B) = ⟨d, v⟩ = ⟨d, c⟩ + ⟨d, \\sgn(d) * (r, …, r)ᵀ⟩ = ⟨d, c⟩ + r · ∑_i^n |d_i|
```
...
function ρ(d::AbstractVector{N}, B::BallInf{N}) where {N<:Real}
    ...
    if length(d) > 30
        ...
        return dot(d, center(B)) + B.radius * sum(abs, d)
    end
    c = center(B)
    res = zero(N)
